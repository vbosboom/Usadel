%Procedure for calculation of element matrices and vectors in the
%1D Usadel weak formulation using the phi-parametrization
function [S1,S2,S3,f] = BuildMatricesandVectors(GI,weights,gamma,omega,phase,phi,Delta_0)
    ...
    for ind1 = 1:GI.topology
        felem = GenerateElementVector(GI,weights,Gammainv,omega,G,phase,Delta_0,h,ind1);
        for ind2 = 1:GI.topology
            [S1elem,S2elem,S3elem] = GenerateElementMatrix(GI,weights,Gammainv,omega,G,phase,Delta_0,GradPhi,h,ind1,ind2);
