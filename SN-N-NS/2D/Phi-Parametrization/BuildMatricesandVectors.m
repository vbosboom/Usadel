%Procedure for the calculation of element matrices and vectors in the
%2D Usadel weak formulation using the phi-parametrization
function [S1,M1,f1,S2,M2,f2,S3,M3,f3] = BuildMatricesandVectors(GI,points,weights,points1D,weights1D,gamma_B,gamma,omega,ksi,phis1,deltas1,phis2,deltas2,phin)
    ...
    for ind1 = 1:GI.topology
        felem = GenerateElementVectorS1(GI,points,weights,omega,G,Delta,ind1);
        for ind2 = 1:GI.topology
            [S1elem,M1elem] = GenerateElementMatrixS1(GI,points,weights,omega,ksi,G,ind1,ind2);
    ...
    for ind1 = 1:GI.topology
        felem = GenerateElementVectorS2(GI,points,weights,omega,G,Delta,ind1);
        for ind2 = 1:GI.topology
            [S2elem,M2elem] = GenerateElementMatrixS2(GI,points,weights,omega,ksi,G,ind1,ind2);
    ...
    for ind1 = 1:GI.topology
        for ind2 = 1:GI.topology
            [S3elem,M3elem] = GenerateElementMatrixN(GI,points,weights,omega,G,ind1,ind2);
    ...
    for ind1 = 1:GI.topologybnd
        [f1Belem,f3Belem] = GenerateBoundaryElementVectorSN(points1D,weights1D,gamma_B,gamma,ksi,lek,PhiS,Gs,PhiN,Gn,ind1);
        for ind2 = 1:GI.topologybnd
            [M1Belem,M3Belem] = GenerateBoundaryElementMatrixSN(points1D,weights1D,gamma_B,gamma,ksi,lek,Gs,Gn,ind1,ind2);
    ...
    %Length of elements on S2-N boundary
    lek = sqrt((GI.x(GI.elmatbnds2(:,2))-GI.x(GI.elmatbnds2(:,1))).^2+(GI.y(GI.elmatbnds2(:,2))-GI.y(GI.elmatbnds2(:,1))).^2).';
    
    %Generate element matrices and vectors on S2-N boundary
    for ind1 = 1:GI.topologybnd
        [f2Belem,f3Belem] = GenerateBoundaryElementVectorSN(points1D,weights1D,gamma_B,gamma,ksi,lek,PhiS,Gs,PhiN,Gn,ind1);
        for ind2 = 1:GI.topologybnd
            [M2Belem,M3Belem] = GenerateBoundaryElementMatrixSN(points1D,weights1D,gamma_B,gamma,ksi,lek,Gs,Gn,ind1,ind2);
